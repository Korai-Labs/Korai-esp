...
static void gpio_usb_uart_draw_callback(Canvas* canvas, void* _model) {
    ...
    canvas_draw_str(canvas, 2, 9, "USB serie");
    canvas_draw_str(canvas, 3, 25, "TX:");
    canvas_draw_str(canvas, 3, 42, "RX:");

    canvas_set_font(canvas, FontSecondary);
    snprintf(temp_str, 18, "Puerto COM:%u", model->vcp_port);
    ...
    if(model->baudrate == 0)
        snprintf(temp_str, 18, "Baudios: ????");
    else
        snprintf(temp_str, 18, "Baudios: %lu", model->baudrate);
